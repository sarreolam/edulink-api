...
app.post("/api/log-in", async (req, res)=>{
  ...
  if (!user){
    res.status(200).send({errorMessage: "No existe este usuario"})
    return
  }
  if (!req.body.password){
    res.status(200).send({errorMessage: "No hay contaseña"})
    return
  }
  if (user.password != req.body.password){
    res.status(200).send({errorMessage: "La contraseña es incorrecta"})
    return
  }
  res.send({data:user})
})
app.post("/api/sign-in", async (req, res)=>{
  if (!req.body.user){
    res.status(200).send({errorMessage: "No hay usuario"})
    return
  }
  if (!req.body.password){
    res.status(200).send({errorMessage: "No hay contaseña"})
  ...
  if (!req.body.email){
    res.status(200).send({errorMessage: "No hay email"})
    return
  }
  const userCheck = await User.findOne({name: req.body.user }).exec()
  if (userCheck){
    res.status(200).send({errorMessage: "Usuario ya existe"})
    return
  }
